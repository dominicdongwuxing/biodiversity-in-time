...
function MyaMainScrollBar ({ handleChange, myaMain }) {
  ...
  useEffect (()=>{
    const slider = sliderBottom()
      ...
      .default(myaMain)
      .on("onchange", val => {handleChange(val)})
...
function MyaRangeScrollBar ({ handleChange, myaMain, steps }) {
  ...
  useEffect (()=>{
    const slider = sliderBottom()
      ...
      .tickFormat(d3.format("d"))
      .ticks((timeRange[1] - timeRange[0])/50)
      .default([(myaMain - steps) < 0 ? 0 : myaMain - steps, (myaMain + steps) > 1000 ? 1000 : myaMain + steps])
      .fill("#2196f3")
...
export default function ScrollBar( { changeMyaMain, changeMyaRange, myaMain, myaRange, steps } ) {
  return (
    <div className={styles.scrollBar}>
      ...
      Fine tune the range around the mya value to view the tree: from {myaRange[0]} mya to {myaRange[1]} mya:
      <MyaRangeScrollBar handleChange={changeMyaRange} myaMain = {myaMain} steps = {steps}/>
